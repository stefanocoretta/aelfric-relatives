...
relatives_counts <- relatives_ehep %>%
  ...
  count(dem, position, is_called, grand_tot) %>%
  drop_na()
```
...
rel_bm_2_cond_l <- rel_bm_2_cond %>%
  ...
  separate(
    condition,
    c("dem", "position", "is_called"),
    sep = "\\."
  ) %>%
  arrange(dem, position, is_called)
```
...
rel_bm_2_cond_l %>%
  ...
  facet_grid(is_called ~ position, labeller = label_both) +
